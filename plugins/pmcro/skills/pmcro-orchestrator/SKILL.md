...
6. Dispatch typed PhaseMessages; never use slash-command text as the inter-agent protocol.
...
Use `.pmcro/schemas/` for IntentEnvelope, PlanEnvelope, CheckReport, ReflectionFrame, OrchestratorDecision, TrailEvent, PhaseMessage, TrailFrame, and DomainRequest. Planner must complete seed-to-true-intent extraction before producing a PlanEnvelope.
